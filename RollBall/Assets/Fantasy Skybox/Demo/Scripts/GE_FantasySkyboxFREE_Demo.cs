...
public class GE_FantasySkyboxFREE_Demo : MonoBehaviour
{
    ...
    private void SwitchSkyBox(int DiffNum)
    {
        ...
        // Switch light
        for (var i = 0; i < m_LightAndSkyList.Length; i++)
        {
    ...
    [Serializable]
    // Embed this class with sub properties in the inspector. http://docs.unity3d.com/ScriptReference/Serializable.html
    ...
    {
        // Update ItemNum text
        var Text_ItemNum = GameObject.Find("Text_ItemNum");
        if (Text_ItemNum != null)
        {
            var pText = Text_ItemNum.GetComponent<Text>();
            pText.text = string.Format("{0:00} of {1:00}", m_CurrentSkyBox + 1, m_LightAndSkyList.Length);
        }

        // Update Details text
        var Text_Details = GameObject.Find("Text_Details");
        if (Text_Details != null)
        {
            var pText = Text_Details.GetComponent<Text>();
    ...
    private void UpdateHowToText()
    {
        ...
        var Text_HowTo = GameObject.Find("Text_HowTo");
        if (Text_HowTo != null)
        {
            ...
            {
                var pText = Text_HowTo.GetComponent<Text>();
